...
class FriendController extends Controller {
    public function index() {
        ...
        $pendingRequests = Friendship::where('friend_id', Auth::id())
            ...
            ->with('user')
            ->get();
        $friendCapsules = collect();
        foreach ($friends as $friendship) {
            $capsules = $friendship->friend->capsules()
                ->where('visibility', 'friends')
                ->where('is_locked', false)
                ->with('user', 'reactions')
                ->get();
            $friendCapsules = $friendCapsules->merge($capsules);
        }
        $friendCapsules = $friendCapsules->sortByDesc('created_at');
    ...
    public function sendRequest(User $user) {
        $exists = Friendship::where('user_id', Auth::id())
            ->where('friend_id', $user->id)->exists();
        if (!$exists) {
            Friendship::create([
                'user_id' => Auth::id(),
                'friend_id' => $user->id,
